 ...
 ID=cat(2,board_I_ID,board_O_ID);
  console_in_length = 1024*2+sum(board_I_config(2,1:end).*board_I_config(3,1:end))*2 + 12;%6 = 2 length + 4 crc32
  console_in_addr  =   0;
  console_out_length = sum(board_O_config(2,1:end).*board_O_config(3,1:end))*2 + 12;%6 = 2 length + 4 crc32
...
for i=1:array_length(1,2)
    ...
    xfer_in(2,AA+j) = (board_I_ID(i)-25)*64 +768+(j-1)*board_I_config(2,i);
...
for i=1:array_length(1,2)
    ...
    xfer_out(1,AA+j) = board_O_config(2,i)-1+112;
    xfer_out(2,AA+j) = temp;
    xfer_out(3,AA+j) = 16+board_O_config(2,i)-1;
    xfer_out(4,AA+j) = (board_O_ID(i)-25)*16+ 768+(j-1)*board_O_config(2,i);
...
for i=1:7
    flash_data(1,1025+(i-1)*4:1025+i*4-1)=[bitand(console(1,i),255) bitand(console(1,i),65280)/256 bitand(console(1,i),16711680)/65536 0 ];
end
flash_data(1053:1056)= CRC_console_in;
for i=1:7
    flash_data(1,1057+(i-1)*4:1057+i*4-1)=[bitand(xfer(1,i),255) bitand(xfer(1,i),65280)/256 bitand(xfer(1,i),16711680)/65536 0 ];
end
flash_data(1,1085:1088)= CRC_console_out;
% % % % % 
for i=1:sum(board_I_config(3,1:end))
    flash_data(1,1089+(i-1)*6:1089+i*6-1)=[bitand(xfer_in(2,i),255) bitand(xfer_in(2,i),65280)/256 bitand(xfer_in(1,i),255) bitand(xfer_in(4,i),255) bitand(xfer_in(4,i),65280)/256 bitand(xfer_in(3,i),255)];
end
...
for i=1:sum(board_O_config(3,1:end))
    ...
    [bitand(xfer_out(2,i),255) bitand(xfer_out(2,i),65280)/256 bitand(xfer_out(1,i),255) bitand(xfer_out(4,i),255) bitand(xfer_out(4,i),65280)/256 bitand(xfer_out(3,i),255)];
end
...
%%
m_bus_data=zeros(1,mem_size*1024*1024/128*138);
for i=1:mem_size*1024*1024/128
    m_bus_data((i-1)*138+1:i*138) = [0 254 96 2 1 mod(i,256)  bitand((i-1)*128,255) bitand((i-1)*128,65280)/256 bitand((i-1)*128,16711680)/65536 np811_type flash_data(1,(i-1)*128+1:i*128) ];
end
% % % % % % 
% g=[1 0 0 1 1];
% R=length(g)-1;
...
%%
 a=(zeros(1,32768));
 LL=1;
 a(2049:2049+LL*2-1)=[1 1];
 for i=2049:2:32767
     a(i+1)=sum(mod(floor(a(i)./2.^(1:8)),2))*2^4+a(i+1);
 end
a=uint8(a);
m_bus_data=zeros(1,248*138);
for i=1:248
    m_bus_data((i-1)*138+1:i*138) = [0 254 96 2 1 mod(i,256) bitand((i-1)*128,255) bitand((i-1)*128,65280)/256 bitand((i-1)*128,16711680)/65536 np811_type a(1,(i-1)*128+1:i*128) ];
end
...
a=uint8(a);
m_bus_data=[0 254 96 2 1 1 bitand(2048,255) bitand(2048,65280)/256 bitand(2048,16711680)/65536 np811_type a ];
